  ...
  return (
    <div className="mb-24 md:mb-32 grid grid-cols-12 gap-8 items-center">
      ...
      <motion.div 
        ...
        className={`col-span-12 md:col-span-6 ${reversed ? 'md:order-last' : 'md:order-first'} flex justify-center`}
      >
        <div className="w-full max-w-[320px] sm:max-w-[400px] md:max-w-none aspect-[4/3] overflow-hidden rounded-xl border border-charcoal/5 shadow-2xl shadow-charcoal/10">
          <img 
            src={image} 
            alt={title} 
            className="w-full h-full object-cover brightness-100 hover:scale-105 transition-all duration-1000"
      ...
      <motion.div 
        ...
        viewport={{ once: true }}
        transition={{ duration: 0.8 }}
        className={`col-span-12 md:col-span-6 ${reversed ? 'md:order-first' : 'md:order-last'}`}
      >
        ...
        <div className="space-y-4 border-t border-charcoal/10 pt-4 px-4 md:px-0">
          {sections.map((section) => (
            <div key={section.id} className="border-b border-charcoal/5 last:border-none pb-4 px-2">
              ...
              >
                <span className={`text-sm uppercase tracking-widest font-bold transition-colors ${openSection === section.id ? 'text-sunset' : 'group-hover:text-sunset'}`}>
                  {section.label}
                </span>
                {openSection === section.id ? (
                  <ChevronDown size={18} className="text-sunset" />
                ) : (
                  <Plus size={18} className="text-charcoal/20 transition-colors group-hover:text-sunset mr-1" />
                )}
              </button>
              <AnimatePresence>
                {openSection === section.id && (
                  ...
                  >
                    <p className="pb-4 text-charcoal/80 leading-relaxed font-sans pr-4">
                      {section.text}
                    </p>
                  </motion.div>
                )}
  ...
  const projects: ProjectAccordionProps[] = [
    {
      ...
      content: {
        ...
        outcome: "Attracted 30,000+ attendees and generated over 100 million impressions through coordinated global media and ecosystem campaigns."
      }
    },
    {
      ...
      content: {
        ...
        outcome: "Drove 300% social growth within six months while building regional partnerships, business opportunities, and market visibility across Singapore and Thailand."
      }
    },
    {
      ...
      content: {
        ...
        outcome: "Successfully supported the delivery of enterprise-level cooperation projects contributing to over $1.2M in annual commercial revenue."
      }
    },
    {
      ...
      content: {
        ...
        outcome: "Mobilized over $600K in ecosystem support and attracted 300+ developer teams to drive early-stage product adoption."
      }
    }
  ];
      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-16 md:mb-24">
          ...
          <motion.h2 
            ...
            className="text-3xl sm:text-4xl md:text-6xl font-bold max-w-[15ch] leading-none"
          >
